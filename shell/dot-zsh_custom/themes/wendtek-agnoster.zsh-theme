...
#
# Wendtek customized Agnoster theme with shorted working directory segments
# Based on the oh-my-zsh vendored agnoster's Theme - https://github.com/ohmyzsh/ohmyzsh/blob/bd295c014f47ac2462d6145f87ae627ae765860f/themes/agnoster.zsh-theme
# Customizations:
# - Shortened current working directory segments to first character except for last segment
# - Remove spaces around segment separators
#
# # README
#
...
# Abbreviate path
# Given a path, abbreviate all but the last segment to their first character
abbreviate_path() {
  local input_path="$1"
  
  # Handle empty, root, or home-only paths
  ...
  path_segments=(${(s:/:)input_path})
  
  # Handle paths with fewer than 2 segments
  if [[ ${#path_segments} -le 1 ]]; then
    echo "$input_path"
    return
  fi
  ...
  local i
  for i in {$start_index..$((${#path_segments}))}; do
    local segment="${path_segments[i]}"
    if [[ -n "$segment" ]]; then
      if [[ $i -eq ${#path_segments} ]]; then
...
prompt_git_relative() {
  ...
  if [[ $path_in_repo != '' ]]; then
    # Use abbreviated path for git relative path too
    prompt_segment "$AGNOSTER_DIR_BG" "$AGNOSTER_DIR_FG" "$(abbreviate_path "$path_in_repo")"
  fi;
}
...
prompt_dir() {
  if [[ $AGNOSTER_GIT_INLINE == 'true' ]] && $(git rev-parse --is-inside-work-tree >/dev/null 2>&1); then
    ...
    local git_root="$(git_toplevel | sed "s:^$HOME:~:")"
    prompt_segment "$AGNOSTER_DIR_BG" "$AGNOSTER_DIR_FG" "$(abbreviate_path "$git_root")"
  else
    # Use abbreviated path instead of %~ since we can't rely on ZSH prompt expansion here
    local current_path="${PWD/#$HOME/~}"
    prompt_segment "$AGNOSTER_DIR_BG" "$AGNOSTER_DIR_FG" "$(abbreviate_path "$current_path")"
  fi
}
